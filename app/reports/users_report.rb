...
class UsersReport < BaseReport
  def call
    result[:users] = []
    received_service_jobs = ServiceJob.where(created_at: period)
    ...
    if received_service_jobs.any?
      received_service_jobs.order('count_id desc').group('user_id').count('id').each_pair do |key, val|
        if key.present? && (user = User.find_by(id: key)).present?
          ...
          result[:users] << { name: user.short_name,
                              qty: val,
                              qty_done: service_jobs.at_done(user.department).count,
                              qty_archived: service_jobs.at_archive(user.department).count,
