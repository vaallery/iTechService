    ...
    product_groups.collect do |product_group, sub_product_groups|
      ...
      if (product_ids = group_store_items.collect{|si|si.product.id}).present?
        products = product_group.products.find(product_ids).collect do |product|
          ...
          {type: 'product', depth: product_group.depth+1, id: product.id, code: product.code, name: product.name, quantity: product_quantity, details: items, purchase_price: product.purchase_price.to_f, price: product.retail_price.to_f, sum: product.retail_price.to_f*product_quantity}
        end
      else
        products = []
      end
