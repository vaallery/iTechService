module RepairGroupsHelper
  ...
  def nested_repair_groups_list(repair_groups, current_id=nil, options={})
    repair_groups.map do |repair_group, sub_repair_groups|
      ...
      content_tag :li, link_to(repair_group.name, repair_group_path(repair_group, options), remote: true) + content_tag(:ul, nested_repair_groups_list(sub_repair_groups, current_id, options)), class: "repair_group #{li_class}", id: "repair_group_#{repair_group.id}", title: repair_group.name, data: {repair_group_id: repair_group.id, depth: repair_group.depth}
    end.join.html_safe
  end
