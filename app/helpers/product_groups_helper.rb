module ProductGroupsHelper
  ...
  def nested_product_groups_list(product_groups, opened=[], current_id=nil, options={})
    ...
    product_groups.map do |product_group, sub_product_groups|
      ...
      content_tag(:li, link_to(product_group.name, product_group_path(product_group, options), remote: true) + content_tag(:ul, nested_product_groups_list(sub_product_groups, opened, current_id, options)), class: "product_group #{li_class}", id: "product_group_#{product_group.id}", title: product_group.name, data: {product_group_id: product_group.id, depth: product_group.depth, products: product_items(product_group.products)})
