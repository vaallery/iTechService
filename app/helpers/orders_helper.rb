module OrdersHelper
  ...
  def order_row_tag(order)
    content_tag(:tr, class: row_class_for_order(order), data: {order_id: order.id}) do
      content_tag(:td, link_to(order.number, order_path(order), target: '_blank'), class: 'order_number_column') +
      ...
      content_tag(:td, class: 'order_customer_column') do
        if order.customer_type == 'User'
          ...
          (order.customer.present? ? link_to(order.customer_presentation, client_path(order.customer), target: '_blank') : '-')
        end.html_safe +
        tag(:br, false) +
        order.comment
      end +
      ...
      content_tag(:td, class: 'order_actions_column') do
        link_to(icon_tag('edit'), edit_order_path(order), class: 'btn btn-small', target: '_blank')
      end
    end.html_safe
  end
