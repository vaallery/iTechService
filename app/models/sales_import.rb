    ...
    9.upto sheet.last_row-1 do |r|
      ...
      case row_type row[0]
        ...
        when :date
          ...
          qty = row[3].to_i
          sale = Sale.find_or_initialize_by_serial_number serial_number: sn, imei: imei, sold_at: date.to_date,
                                                          device_type_id: device_type.try(:id), quantity: qty
          unless !sale.new_record? and sale.sold_at == date.to_date
            sales << sale
            import_logs << ['success', (sale.new_record? ? 'New' : 'Existing') +
                " device #{device_type.try(:full_name)} [#{sale.serial_number}] sold_at: #{sale.sold_at}"]
          end
        else
          # type code here
      end
